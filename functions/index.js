...
/**
 * HTML personnalisé Firestore (`userAcknowledgmentHtml` = FR) ou variante par locale
 * (`userAcknowledgmentHtml_en`, …). Hors FR sans variante → HTML intégré i18n.
 * @param {Record<string, unknown>} tplData
 * @param {string} locale
 */
function paychekUserAcknowledgmentHtmlOverride(tplData, locale) {
  const loc = emailI18n.normalizePaychekEmailLocale(locale);
  const perLocale = `${tplData[`userAcknowledgmentHtml_${loc}`] ?? ""}`.trim();
  if (perLocale) return perLocale;
  if (loc === "fr") {
    return `${tplData.userAcknowledgmentHtml ?? ""}`.trim();
  }
  return "";
}
...
async function resolveUserAcknowledgmentHtml(db, ticketData, ticketLabels, locale) {
  ...
  try {
    ...
    custom = paychekUserAcknowledgmentHtmlOverride(data, loc);
  } catch (e) {
...
const coachAi = require("./coach_ai");
Object.assign(
    exports,
    coachAi.createCoachAiExports({
      onCall,
      HttpsError,
      admin,
      paychekTrialRemainderMsForUid,
    }),
);
...
const webContact = require("./web_contact");
Object.assign(
    exports,
